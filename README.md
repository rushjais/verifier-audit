...
tried, three of those strategies rank a blank screen above a correct implementation.
Not an adversarial input, just two conventions that disagree about one instruction. A follow-up study
...
SUPER-CHIP shift VX in place. Real interpreters do both. So the glyph lands four pixels apart, and the grader ranks that legitimate difference *below* a submission that renders nothing.

**Why this matters for RL:** a grader like this is the reward signal. Ranking a correct
implementation below one that does nothing doesn't waste that rollout's compute.
It spends it
...
a completely different instruction.
`BNNN` adds `V0` on the VIP and `VX` on CHIP-48, so the two
...
## Where the failure stops, and what I got wrong
...
predicted them in advance to ±0.04.
Three landed inside ±0.01, the fourth missed its tolerance by
...
| **Upstream** | 6 bug reports filed across 4 repos, all still open, plus [a pull request](https://github.com/wyattferguson/chip8-emulator/pull/9) fixing a 7th. It takes that interpreter from 8 failures to 0 on the reference suite. 1 defect remains unreported (`debugloop`: issues disabled, dormant since 2022) |
...
- **Two of the five ROMs are inert as population tests.** On D and E all seven interpreters agree,
  ...
  **three** of the five documented quirks.
  The quirks that divide real implementations are a
...
process boundary between the candidate and the answers:
the child receives only the inputs.

**Authorship.** Written with an AI coding assistant under my direction. I set the direction, made
the scoping calls, reviewed the output and pushed back on it,
