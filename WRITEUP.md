...
evaporate at 256. Only the write-order defect was real. **The reasoning was cited as evidence
against someone else's project**, which is the worst place for it, and it survived because a
constant that sets the whole conclusion was never looked up.
...
| --- | --- | --- |
| craigthomas | **0** |, |
| wyattferguson | 8 | `cpu.py:157-160`, VF written before VX, so `8XY4` into VF clobbers the carry |
...
hypothetical. The GMSD inversion result is a **known property of the measure**, not a discovery, gradient magnitude is invariant under inversion by construction, though it does not appear to be
...
moving the glyph four pixels disturbs **two** blocks, the one it left and the one it entered, while erasing it entirely disturbs **one**. A metric that pools over blocks therefore penalises
...
The failure is specific to **spatial displacement**, and the mechanism given above is the reason, moving a glyph disturbs the block it left *and* the block it entered, while deleting or
...
Under the strict rule, scored against `craigthomas` alone, three of the five find a separating
threshold: exact match, pixel proportion and SSIM. Under the extended rule, over all seven
interpreters, **none of the five does.**
...
eligibility rule was amended twice, both before any metric existed, both recorded with reasoning, including that Amendment 1 was made *after* the strict rule returned an unusable answer, by the
...
so that command set a config pointing at nothing and the gate had never run in this repository, incident #17. The hook exists as of this commit and runs `make check` unpiped, which is #3. And
